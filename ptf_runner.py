...
ptf.testutils.TEST_PARAMS = params_parse(config)
...
LOGGER.info('test_modules: ' + str(test_modules))
...
group or their module's test group.""")
    ...
    for (modname, (mod, tests)) in test_modules.items():
        ...
        LOGGER.info(start_str + _space_to(22, start_str) + desc)
        for (testname, test) in list(tests.items()):
            ...
            test_count += 1
        LOGGER.info()
    LOGGER.info("%d modules shown with a total of %d tests" % \
        (mod_count, test_count))
    LOGGER.info()
    LOGGER.info("Test groups: %s" % (', '.join(sorted(all_groups))))
...
if config["list_test_names"]:
    for (modname, (mod, tests)) in test_modules.items():
        for (testname, test) in tests.items():
            LOGGER.info("%s.%s" % (modname, testname))
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    if ptf.testutils.skipped_test_count > 0:
        ...
        LOGGER.info("Skipped " + str(ptf.testutils.skipped_test_count) + ts)
    ...
    if run_failures or run_errors:
        LOGGER.info()
        LOGGER.info("******************************************")
        LOGGER.info("ATTENTION: SOME TESTS DID NOT PASS!!!")
        if (not config["xunit"]) and run_failures:
            LOGGER.info()
            LOGGER.info("The following tests failed:")
            LOGGER.info(", ".join([f[0].__class__.__name__ for f in run_failures]))
        if (not config["xunit"]) and run_errors:
            LOGGER.info()
            LOGGER.info("The following tests errored:")
            LOGGER.info(", ".join([f[0].__class__.__name__ for f in run_errors]))
        if (not config["xunit"]) and run_timeouts:
            LOGGER.info()
            LOGGER.info("The following tests errored because of a timeout:")
            LOGGER.info(", ".join([f[0].__class__.__name__ for f in run_timeouts]))
        LOGGER.info()
        LOGGER.info("******************************************")
